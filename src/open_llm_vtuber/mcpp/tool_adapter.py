...
class ToolAdapter:
    ...
    def format_tools_for_api(
        ...
        for tool_name, data_object in formatted_tools_dict.items():
            ...
            # Prefix tool name with server name to avoid reserved name conflicts
            # (e.g., DashScope/Qwen disallows tool names like "search")
            server_name = data_object.related_server
