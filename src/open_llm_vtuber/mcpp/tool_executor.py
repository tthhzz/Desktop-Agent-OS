...
class ToolExecutor:
    ...
    async def run_single_tool(
        ...
        # Strip server prefix if present (format: "server__tool")
        # ToolManager stores tools by their original name, but API uses prefixed names
        lookup_name = tool_name
        if "__" in tool_name:
            parts = tool_name.split("__", 1)
            lookup_name = parts[1]
            logger.debug(f"Stripped prefix from '{tool_name}' to '{lookup_name}' for lookup")

        tool_info = self._tool_manager.get_tool(lookup_name)
