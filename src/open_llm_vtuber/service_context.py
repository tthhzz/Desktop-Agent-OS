...
class ServiceContext:
    ...
    def __init__(self):
        ...
        # Memory system for agents that support it (e.g., langgraph_agent)
    ...
    def _get_mcp_settings(self) -> tuple:
        """Get MCP use_mcpp and mcp_enabled_servers from the active agent config."""
        agent_settings = self.character_config.agent_config.agent_settings

        if agent_settings.langgraph_agent:
            return (
                agent_settings.langgraph_agent.use_mcpp,
                agent_settings.langgraph_agent.mcp_enabled_servers,
            )
        return False, []

    async def _init_mcp_components(self, use_mcpp=None, enabled_servers=None):
        """Initializes MCP components based on configuration, dynamically fetching tool info."""
        # If called without explicit args, get from config
        if use_mcpp is None:
            use_mcpp, enabled_servers = self._get_mcp_settings()
        logger.debug(
    ...
    async def load_from_config(self, config: Config) -> None:
        ...
        use_mcpp, _ = self._get_mcp_settings()
    ...
    async def init_agent(self, agent_config: AgentConfig, persona_prompt: str) -> None:
        ...
        # Initialize memory system for agents that support it
        if agent_config.conversation_agent_choice == "langgraph_agent":
            if not self.memory_system:
                try:
                    self.memory_system = MemorySystem()
                    await self.memory_system.initialize()
                    logger.info("MemorySystem initialized for langgraph_agent")
                except Exception as e:
