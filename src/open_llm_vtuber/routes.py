...
def init_webtool_routes(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def upload_file(
        ...
        client_uid: str = Form(None),
    ):
        """
        Upload a file and return base64-encoded data.

        The returned data can be included in a subsequent text-input WebSocket
        message as the ``files`` field.

        Form fields:
        - file: The file to upload
        - client_uid: Optional client UID to associate with the upload
        """
        ...
        try:
            ...
            logger.info(
                f"File uploaded successfully: {file.filename} ({len(contents)} bytes)"
            )
            return result
