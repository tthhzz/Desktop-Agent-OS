...
class WebSocketHandler:
    ...
    async def _handle_approval_response(
        ...
        """Handle user approval/rejection of a pending tool call.
        ...
        logger.info(f"Received approval response from {client_uid}: {response}")

        # Forward the approval to the message handler so the
        # LangGraphAgent can pick it up when it resumes.
        from .message_handler import message_handler
        message_handler.handle_message(client_uid, {
            "type": "approval-response",
            "response": response,
        })
